...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get slug from query (Vercel passes dynamic route params in req.query)
    const { slug } = req.query;
    console.log('Reviews API - Request query:', req.query);
    console.log('Reviews API - Slug:', slug);
    
    if (!slug) {
      console.error('Reviews API - Slug missing from query');
